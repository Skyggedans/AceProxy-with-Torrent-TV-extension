...
from torrenttv.torrenttv import TorrentTV
...
class AceHandler(BaseHTTPServer.BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    try:
      ...
      if not self.path.split('/')[1].lower() in ('pid', 'torrent', 'tortvplaylist') or not self.path.split('/')[2]:
	self.die_with_error()
	return
    except IndexError:
      self.die_with_error()
    ...
    elif self.reqtype == 'tortvplaylist':
	playlist = TorrentTV.getPlaylist(type = self.path_unquoted)
	if playlist:
	    self.send_response(200)
            if self.path_unquoted == 'xml':
